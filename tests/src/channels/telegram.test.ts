...
vi.mock('grammy', () => ({
  Bot: class MockBot {
    ...
    start(opts: { onStart: (botInfo: any) => void }) {
      opts.onStart({ username: 'tg_james_bot', id: 12345 });
    }
...
describe('TelegramChannel', () => {
  ...
  describe('@mention translation', () => {
    it('translates @bot_username mention to trigger format', async () => {
      ...
      const ctx = createTextCtx({
        text: '@tg_james_bot what time is it?',
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({
          content: '@UnitTestNameBob @tg_james_bot what time is it?',
        }),
      );
    });
    ...
    it('does not translate if message already matches trigger', async () => {
      ...
      const ctx = createTextCtx({
        text: '@UnitTestNameBob @tg_james_bot hello',
        entities: [{ type: 'mention', offset: 17, length: 13 }],
      });
      await triggerTextMessage(ctx);

      // Should NOT double-prepend — already starts with @UnitTestNameBob
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({
          content: '@UnitTestNameBob @tg_james_bot hello',
        }),
      );
    });
    ...
    it('handles mention in middle of message', async () => {
      ...
      const ctx = createTextCtx({
        text: 'hey @tg_james_bot check this',
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        'tg:100200300',
        expect.objectContaining({
          content: '@UnitTestNameBob hey @tg_james_bot check this',
        }),
      );
    });
