...
export const ASSISTANT_NAME = process.env.ASSISTANT_NAME || 'Echo';
...
Messages must start with the trigger pattern (default: `@Echo`):
- `@Echo what's the weather?` → ✅ Triggers Claude
- `@echo help me` → ✅ Triggers (case insensitive)
- `Hey @Echo` → ❌ Ignored (trigger not at start)
...
[Jan 31 2:35 PM] John: @Echo what toppings do you recommend?
```
...
|---------|---------|--------|
| `@Assistant [message]` | `@Echo what's the weather?` | Talk to Claude |

### Commands Available in Main Channel Only

| Command | Example | Effect |
|---------|---------|--------|
| `@Assistant add group "Name"` | `@Echo add group "Family Chat"` | Register a new group |
| `@Assistant remove group "Name"` | `@Echo remove group "Work Team"` | Unregister a group |
| `@Assistant list groups` | `@Echo list groups` | Show registered groups |
| `@Assistant remember [fact]` | `@Echo remember I prefer dark mode` | Add to global memory |
...
```
User: @Echo remind me every Monday at 9am to review the weekly metrics
...
```
User: @Echo at 5pm today, send me a summary of today's emails
...
From any group:
- `@Echo list my scheduled tasks` - View tasks for this group
- `@Echo pause task [id]` - Pause a task
- `@Echo resume task [id]` - Resume a paused task
- `@Echo cancel task [id]` - Delete a task

From main channel:
- `@Echo list all tasks` - View tasks from all groups
- `@Echo schedule task for "Family Chat": [prompt]` - Schedule for another group
...
| "No groups registered" | Haven't added groups | Use `@Echo add group "Name"` in main |
